...
function ImageSlider({ route: { params: { images, initialIndex = 0 } } }: Props): React.ReactElement {
  ...
  const translateOtherImages = React.useCallback(() => {
    animValues.nextTranslateX = (animValues.scale - 1) * imageContainerWidth / 2 + animValues.x;
  ...
  const onRelease = React.useCallback(() => {
    ...
    if (moveNext && currentIndex < images.length - 1 && targetTranslate) {
      Animated.timing(targetTranslate, {
        toValue: { x: -(animValues.scale + 1) / 2 * imageContainerWidth, y: animValues.y },
      ...
      Animated.timing(targetTranslate, {
        toValue: { x: (1 - animValues.scale) * imageContainerWidth / 2, y: animValues.y },
        useNativeDriver: true,
        duration: 300,
      }).start(() => {
      ...
      Animated.timing(targetTranslate, {
        toValue: { x: (animValues.scale - 1) * imageContainerWidth / 2, y: animValues.y },
        useNativeDriver: true,
        duration: 300,
      }).start(() => {
        pinchZoom.current?.setValues({
