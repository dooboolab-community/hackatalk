import icAddW from '../../assets/icons/btnAdd.png';
import icBack from '../../assets/icons/btnBack.png';
import icCamera from '../../assets/icons/camera.png';
import icCircleX from '../../assets/icons/x_circle.png';
import icError from '../../assets/icons/error.png';
import icIcon from '../../assets/icons/icon.png';
import icLogoD from '../../assets/icons/logo_d.png';
import icLogoW from '../../assets/icons/logo_w.png';
import icMsgCamera from '../../assets/icons/msg_camera.png';
import icMsgFile from '../../assets/icons/msg_file.png';
import icMsgImage from '../../assets/icons/msg_image.png';
import icMsgMic from '../../assets/icons/msg_mic.png';
import icMsgPhone from '../../assets/icons/msg_phone.png';
import icNoImage from '../../assets/icons/no_image.png';
import icPic from '../../assets/icons/pictoGallery.png';
import icProfile from '../../assets/icons/profile.png';
import icProfileW from '../../assets/icons/profile_w.png';
import icSearch from '../../assets/icons/search.png';
import icSearchW from '../../assets/icons/search_w.png';
import icSettingW from '../../assets/icons/setting_w.png';
import svgApple from '../../assets/icons/apple.svg';
import svgArrDown from '../../assets/icons/arr_down.svg';
import svgArrUp from '../../assets/icons/arr_up.svg';
import svgCheck from '../../assets/icons/check.svg';
import svgFacebook from '../../assets/icons/facebook.svg';
import svgGoogle from '../../assets/icons/google.svg';
import svgPlus from '../../assets/icons/plus.svg';

export const IC_ADD_W = icAddW;
export const IC_ICON = icIcon;
export const IC_CAMERA = icCamera;
export const IC_CIRCLE_X = icCircleX;
export const IC_ERROR = icError;
export const IC_PROFILE = icProfile;
export const IC_BACK = icBack;
export const IC_PIC = icPic;
export const IC_SEARCH = icSearch;
export const IC_LOGO_D = icLogoD;
export const IC_LOGO_W = icLogoW;
export const IC_MSG_CAMERA = icMsgCamera;
export const IC_MSG_FILE = icMsgFile;
export const IC_MSG_IMAGE = icMsgImage;
export const IC_MSG_MIC = icMsgMic;
export const IC_MSG_PHONE = icMsgPhone;
export const IC_NO_IMAGE = icNoImage;
export const IC_SETTING_W = icSettingW;
export const IC_SEARCH_W = icSearchW;
export const IC_PROFILE_W = icProfileW;
export const SvgApple = svgApple;
export const SvgCheck = svgCheck;
export const SvgGoogle = svgGoogle;
export const SvgFacebook = svgFacebook;
export const SvgPlus = svgPlus;
export const SvgArrUp = svgArrUp;
export const SvgArrDown = svgArrDown;

export default [
  //! Do not run svgs for now
  // SvgApple,
  // SvgCheck,
  // SvgGoogle,
  // SvgFacebook,
  // SvgPlus,
  // SvgArrUp,
  // SvgArrDown,
  IC_ADD_W,
  IC_ICON,
  IC_CAMERA,
  IC_CIRCLE_X,
  IC_ERROR,
  IC_PROFILE,
  IC_BACK,
  IC_PIC,
  IC_SEARCH,
  IC_LOGO_D,
  IC_LOGO_W,
  IC_SETTING_W,
  IC_SEARCH_W,
  IC_PROFILE_W,
];
