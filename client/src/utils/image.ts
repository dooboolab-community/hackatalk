...
export const resizePhotoToMaxDimensionsAndCompressAsPNG =
  async ({
    uri,
    width = 1280,
    height = 1280,
  ...
  // 1. define the maximal dimension and the allowed value for it
    const largestDimension = (width > height) ? PhotoDimensions.WIDTH : PhotoDimensions.HEIGHT;
    const initialValueOfLargestDimension = (await getOriginalImageSize(uri))[largestDimension];
